...
public class HDFSWrite extends Configured implements Tool {
	...
	static class Map extends MapReduceBase implements Mapper<WritableComparable, Writable, BytesWritable, BytesWritable> {
		...
		public void map(WritableComparable key, Writable value, OutputCollector<BytesWritable, BytesWritable> output, Reporter reporter) throws IOException {
			...
			while (numBytesToWrite > 0) {
				...
				int totalBytes = randomKey.getLength() + randomValue.getLength();
				totalBytes = (int) (totalBytes * 1.015);
				numBytesToWrite -= totalBytes;
				reporter.incrCounter(Counters.BYTES_WRITTEN, totalBytes);
	...
	public int run(String[] args) throws Exception {
		...
		/*
		 * int numMaps = (int) (totalBytesToWrite / numBytesToWritePerMap); if
		 * (numMaps == 0 && totalBytesToWrite > 0) { numMaps = 1;
		 * job.setLong("test.randomwrite.bytes_per_map", totalBytesToWrite); }
		 */

		int numMaps = (int) (totalBytesToWrite / numBytesToWritePerMap);
		System.out.println("# of HDFSWrite tasks: " + numMaps);
		job.setNumMapTasks(numMaps);

		System.out.println("client.getClusterStatus().getMaxMapTasks() gives " + cluster.getMaxMapTasks());
		System.out.println("client.getClusterStatus().getMaxReduceTasks() gives " + cluster.getMaxReduceTasks());

		System.out.println("Running on " + cluster.getTaskTrackers() + " nodes with " + numMaps + " maps, \n" + "writing " + totalBytesToWrite + " bytes with " + numBytesToWritePerMap
				+ " bytes per map.");
	...
	public static void main(String[] args) throws Exception {
		int res = ToolRunner.run(new Configuration(), new HDFSWrite(), args);
		System.exit(res);
	}
